...
console.log('loaded polyfill 123');
import io from 'socket.io-client/dist/socket.io';
console.log('loaded io', io);
import CallstatsAPI from 'react-native-callstats/callstats';
console.log('loaded callstats', callstats);

var socket1 = io.connect('https://demo.callstats.io', {
...
console.log('loaded callstats', callStats.version);
          ...
          pc.createAnswer()
            .then(desc => {
              pc.setLocalDescription(desc)
                .then(() => {
                  json = {offer: pc.localDescription};
...
function createPC(socketId, isOffer) {
  ...
  callStats.reportError(
    pc,
    conferenceID,
    callStats.webRTCFunctions.applicationError,
    'Report error',
  );
  callStats.reportError(
    pc,
    ...
    pc.createOffer()
      .then(desc => {
        pc.setLocalDescription(desc)
          ...
          .catch(error1 => {
            console.log('setLocalDescription error', error1);
          });
      })
      .catch(error2 => {
        console.log('createOffer error', error2);
      });
  }
  ...
  pc.onaddstream = function (event) {
    ...
    var remoteList = container.remoteList;
...
socket1.on('connect', function (data) {
  ...
  callStats.initialize(appID, appSecret, localUserID, cscallback, null, {
    collectLegacyStats: false,
  });
});
...
function getStats() {
  ...
  pc.getStats().then(stats => {
    ...
    console.log('stats from getStats ', results);
  });
}

var container;

class RTCDemo extends React.Component {
  constructor(props) {
    super(props);
  ...
  getLocalCameraStream() {
    getLocalStream(true, function (stream) {
      ...
      container.setState({
        status: 'ready',
        info: 'I am in room - ' + container.roomID,
      });
    });
  }
  _switchVideoType() {
  ...
  renderMainContainer() {
    console.log('renderMainContainer renderMainContainer');
    if (!this.joinState) {
      return (
        <View style={styles.container}>
          <View style={{flexDirection: 'row', backgroundColor: '#282849'}}>
            <Image
              ...
              source={{
                uri: 'https://dashboard.callstats.io/static/minimal-logo.png',
              }}
            />
          </View>
  ...
  renderJoinContainer() {
    console.log('renderJoinContainer renderJoinContainer');
    if (this.joinState) {
      return (
        <View style={styles.joinContainer}>
          ...
          />
          <Text style={styles.welcome}>Enter the Room Name to Join/Create</Text>
          <TextInput
          ...
          <TouchableHighlight
            ...
            onPress={e => this.handleJoinClick(e)}>
